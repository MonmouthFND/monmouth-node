...
```rust,ignore
use monmouth_handlers::QmdbHandle;
...
let db = monmouth_handlers::QmdbRefDb::new(handle).expect("tokio runtime");
...
1. **Layer 1 (monmouth-qmdb)**: Pure store logic, state transitions, no synchronization
