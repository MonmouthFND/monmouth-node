...
use monmouth_service::LegacyNodeService;

#[derive(Parser, Debug)]
#[command(name = "monmouth")]
...
impl Cli {
    ...
    fn run_dkg(&self, args: &DkgArgs) -> eyre::Result<()> {
        use monmouth_dkg::{DkgCeremony, DkgConfig};
    ...
    fn run_validator(&self, args: &ValidatorArgs) -> eyre::Result<()> {
        ...
        if !monmouth_dkg::DkgOutput::exists(&config.data_dir) {
            return Err(eyre::eyre!(
                "DKG output not found. Run 'monmouth dkg' first to generate threshold shares."
            ));
        }

        let dkg_output = monmouth_dkg::DkgOutput::load(&config.data_dir)?;
